...
当前项目处于正式开发初始阶段，目标是先建立清晰、可扩展、无演示包袱的基础框架。目前可以接入真实数据源（如直接 HTTP 抓取）和真实大模型服务（如直接调用 API）进行快速开发与验证。
...
- 允许在 tasks 或 service 层直接调用真实 API 或 SDK 获取数据以快速验证。
...
- `backend/app/main.py`：FastAPI 应用入口，挂载 CORS、健康检查和 API v1 路由，管理调度器生命周期。
- `backend/app/core/config.py`：全局配置，基于 Pydantic Settings，从环境变量读取。
- `backend/app/core/logging.py`：Loguru 日志初始化，接管标准库 logging。
- `backend/app/core/scheduler.py`：APScheduler AsyncIOScheduler 实例，提供 `start_scheduler()` / `shutdown_scheduler()` 生命周期接口。
- `backend/app/data/layout.py`：统一管理 `local_data` 目录布局。
- `backend/app/data/market_bar_store.py`：标准化行情 K 线的本地 Parquet 存储。
- `backend/app/data/normalizers.py`：行情字段、时间和空值标准化。
- `backend/app/data/providers/base.py`：真实行情数据源适配接口。
- `backend/app/tasks/example_jobs.py`：示例定时任务，提供 `register_jobs()` 注册入口。
...
- 配置统一从 `backend/app/core/config.py` 获取。
- 日志使用 `loguru`，初始化入口在 `backend/app/core/logging.py`。
- 本地数据路径统一通过 `backend/app/data/layout.py` 获取。
- 定时任务统一在 `backend/app/tasks/` 下定义，通过 `register_jobs()` 注册到调度器；不要在 API 层或 Service 层直接操作调度器。
